...
class Adaptive_Spectral_Block(nn.Module):
    ...
    def forward(self, x_in):
        ...
        weight = torch.view_as_complex(self.complex_weight)  # .view_as_complex
...
class DCT(nn.Module):
    def __init__(self, len):
        super().__init__()
        self.global_weight = nn.Parameter(torch.randn(len, dtype=torch.float32))
        # trunc_normal_(self.global_weight, std=.02)
    ...
    def forward(self, x_in):
        ...
        x_weighted = dct_data * self.global_weight
...
class TSLANet_layer(L.LightningModule):
    def __init__(self, dim, mlp_ratio=3., drop=0., drop_path=0., norm_layer=nn.LayerNorm):
        ...
        # self.asb = Adaptive_Spectral_Block(dim)  # original
        # self.asb = DCT(len=args.emb_dim)  # newFilter
        self.asb = DCT_GRU(dim=dim, GRU_layers=1)  # newFilter
        # self.asb = DCT_GRU_1(dim=105, GRU_layers=1)  # newFilter
        # Worms 224   Yoga 105  ScreenType 179  Haptics 272   InlineSkate 469  Lightning7 78  MedicalImages 23  MiddlePhalanxOutlineCorrect 19
        # OliveOil 141  Adiac 43  ArrowHead 61  Computers 179  ChlorineConcentration 40   CricketX 74  DistalPhalanxOutlineCorrect 19  ElectricDevices 23
        # FaceAll 31  RefrigerationDevices 179

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        # self.norm2_nf = norm_layer(args.seq_len)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_id', type=str, default='UCR_datasets')  
    parser.add_argument('--data_path', type=str, default=r'../dataset/UCR/Yoga')
    ...
    # TSLANet components:
    parser.add_argument('--load_from_pretrained', type=str2bool, default=False, help='False: without pretraining')  # default=True
